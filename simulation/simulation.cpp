...
int main (int argc, char *argv[]) {
    string devices_str = "300"; // tamanho do arquivo de dispositivos.
    ...
    while (!compileResult and devices <= 900) {
        int tech = 2;
        while (tech <= 6) {
            string executeCommand = path + "./solver_model " + to_string (devices) + " " + to_string (tech); system (executeCommand.c_str ()); tech++;
        }
        devices += 200;
    }
    return 0;
